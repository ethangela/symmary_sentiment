...
### Overall workflow
- We employ `sheet.py` to automatically extract and analyse tables, i.e., steps i to vi below
- 1. Automatically extract tables from the PDF using `pdfplumber` pakcage 
- 2. Use custom dictionaries to clarify the information in the extracted tables (e.g., convert YOY to year-on-year, CC to constant currency, ($) to (in doller term) etc.) 
- 3. Extract the information from the tables and embed it using `'all-MiniLM-L6-v2'` model imported from `SentenceTransformer` pakcage
- 4. Also embed the information from the cleaned earning call text, the extracted sub-texts from Sector/Industry Performance Comparisions and Region Comparisons (tasks above)  
- 5. Calculate cosine scores between the two embeddings to identify connections between the descriptions in the earnings call and the information presented in the tables
- 6. List the top statements (with scores) from the earnings call that correspond to the information in each row of the table
...
- All results below are automatically generated by `sheet.py`. I provide explanations for these results with sentences beginning with `(Ethan's explanation)` to clarify their meanings
...
- (Ethan's explanation) The result (top 1) is the corresponding description that closely aligns with the information in the table showing `Q3 revenue declined by 1% quarter-on-quarter and 1% year-on-year`
...
- (Ethan's explanation) The result (top 1) is the corresponding description that closely aligns with the information in the table showing `operating margin was at 20.5% in the third quarter`
...
- (Ethan's explanation) The results (top 1 and top 3) are the corresponding descriptions that closely align with the information in the table showing `Large deal TCV of Q3 was $3.2 billion with 71% net new`
...
- (Ethan's explanation) The results (top 2 and top 4) are the corresponding descriptions closely align with the information in the table showing `Revenue Growth on Q3 2024 is with QoQ growth as Reported -1.2% and CC -1.0%`
...
- (Ethan's explanation) The results (top 1 and top 4) are the corresponding descriptions that closely align with the information in the table showing `Revenue Growth on Q3 2024 is with YoY growth as Reported 0.1% and CC -1.0%`
...
- (Ethan's explanation) The result (top 1) is the corresponding descriptions that closely align with the information in the table showing that `Manufacturing is with strong performance with YoY growth Reported 12.5% and CC 10.6%`
